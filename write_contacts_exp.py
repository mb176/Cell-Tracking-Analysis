...
deltaT = 3 # Number of time steps that we consider to determine "instantaneous" velocity
only_full_duration_tracks= True # Exclude reference and contact tracks that are not there for the entire simulation
...
contact_ended = False
with open(outputFolder+subfolder+f"/contacts_deltaT_{deltaT}_selected_new_log.csv", 'w') as log:
    with open(outputFolder+subfolder+f"/contacts_deltaT_{deltaT}_selected_new.csv", 'w') as f:
